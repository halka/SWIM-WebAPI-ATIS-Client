...
test('getAtis returns the original API JSON without transformation', async () => {
  ...
  const result = await client.getAtis<ApiPayload>({ location: ['rjtt', 'RJCC'], dispcnt: 3 });
...
test('requires dispcnt as defined by the ATIS specification', async () => {
  const client = new SwimClient({ session, fetch: async () => assert.fail('fetch must not run') });

  await assert.rejects(
    client.getAtis({ location: 'RJTT', dispcnt: undefined as unknown as number }),
    /1 through 50/,
  );
});
...
test('returns HTTP 200 business-error JSON unchanged', async () => {
  ...
  assert.deepEqual(await client.getAtis({ location: 'RJ88', dispcnt: 1 }), original);
});
...
test('throws for HTTP-level failures', async () => {
  ...
  await assert.rejects(client.getAtis({ location: 'RJTT', dispcnt: 1 }), /403 Forbidden/);
});

test('requires authentication before requesting ATIS data', async () => {
  const client = new SwimClient({ fetch: async () => assert.fail('fetch must not run') });
  await assert.rejects(client.getAtis({ location: 'RJTT', dispcnt: 1 }), /Authentication required/);
});
